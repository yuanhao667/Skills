...
面向最终用户的在线产品优先使用“在线状态”、平台或正式版本等用户能判断的信号；不要仅因技术栈真实存在就展示框架、Runtime、API 或后端徽章。技术徽章只用于开发者、贡献者或 SDK／库／CLI／基础设施受众。

通常使用 1–5 个。不要为了凑数添加徽章，也不要使用无法验证的 `Production Ready`、`Best`、`100%` 等装饰性声明。
...
- 面向最终用户的在线产品默认省略本地开发、API 文档、接口、环境变量、项目结构、技术栈和部署说明。
